      ...
      const updatedCart = prevCart.map((product) => {
        ...
        if (product.product_id === id) {
          return {
            ...product,
            quantity:
              product?.quantity == product?.product_available_quantity
                ? product?.quantity
                : product?.quantity + 1,
          };
        }
          ...
          <tbody>
            {userCart.length !== 0 ? (
              userCart?.map((cartProduct) => (
                <tr key={cartProduct?._id} className="border-b border-gray-300">
                  <td className="flex w-60 py-6 items-center gap-3">
                    <img
                      className="w-12 md:w-20"
                      src={cartProduct?.featured_photo}
                      alt=""
                    />
                  ...
                  <td className="font-semibold">
                    $
                    {cartProduct?.price?.sale_price ||
                      cartProduct?.price?.regular_price}
                  </td>
                  <td>
                    <div className="flex border p-2 w-min border-gray-300 rounded-full justify-center items-center gap-3">
                      <div>
                        <Button
                          onClick={() =>
                            handleQuantityMinus(cartProduct?.product_id)
                          }
                          size="sm"
                          radius="full"
                          variant="flat"
                      ...
                      <div>
                        <Button
                          onClick={() =>
                            handleQuantityPlus(cartProduct?.product_id)
                          }
                          size="sm"
                          radius="full"
                          variant="flat"
                  ...
                  <td>
                    <div className="font-semibold flex justify-between items-center">
                      ...
                      <Button
                        onClick={() =>
                          handleDeleteFromCart(cartProduct?.product_id)
                        }
                        size="sm"
                        radius="full"
                        variant="bordered"
              ...
              <tr>
                <td className="pt-10" colSpan={4}>
                  <h3 className="text-center font-semibold mb-10 md:text-3xl">
