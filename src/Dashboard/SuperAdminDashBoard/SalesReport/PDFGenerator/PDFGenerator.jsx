...
const PDFGenerator = ({ salesReport }) => {
  ...
  useEffect(() => {
    ...
    salesReport?.products?.forEach((product) => {
      ...
      const artistProfit =
        product.profit_distribution.artist_profit_details.artistProfit *
        product.quantity;
      const websiteProfit =
    ...
    axios
      ...
      .then((res) => {
        ...
        axios
          .get(
            `https://mbb-e-commerce-server.vercel.app/prison/${res.data?.billingInfo?.prison?.prison_email}`
          )
          .then((res) => {
            console.log(res.data);
            setPrison(res.data);
            ...
            <View style={styles.section}>
              ...
              <View style={styles.table}>
                ...
                {salesReport?.products?.map((product, index) => (
                  <View key={product?.order_id} style={styles.tableRow}>
                    ...
                    <Text style={styles.tableCell}>
                      ${product?.price?.cost_price * product?.quantity}
                    </Text>
                    <Text style={styles.tableCell}>
                      $
                      {product?.profit_distribution?.artist_profit_details
                        ?.artistProfit * product?.quantity}
                    </Text>
                    <Text style={styles.tableCell}>
                      $
                      {product?.profit_distribution?.website_profit_details
                        ?.websiteProfit * product?.quantity}
                    </Text>
                    <Text style={styles.tableCell}>
                      $
                      {product?.profit_distribution?.prison_profit_details
                        ?.prisonProfit * product?.quantity}
                    </Text>
                  </View>
                ))}
                <View style={styles.tableRow}>
              ...
              <View style={styles.optionsContainer}>
                ...
                <View style={styles.optionColumn}>
                  ...
                  <Text style={styles.optionText}>
                    Balance: ${totalArtistProfit} + ${totalCostPrice} = $
                    {totalArtistProfit + totalCostPrice}
                  </Text>
                  <Text style={styles.optionText}>{artist?.userName}</Text>
                  <Text style={styles.optionText}>
                    {artist?.billingInfo?.address},{" "}
                    {artist?.billingInfo?.states} {artist?.billingInfo?.zipCode}
                  </Text>
                </View>
                <View style={styles.optionColumn}>
                  <Text style={styles.optionHeaderText}>Prison</Text>
                  <Text style={styles.optionText}>
                    Balance: ${totalPrisonProfit}
                  </Text>
                  <Text style={styles.optionText}>{prison?.prison_name}</Text>
                  <Text style={styles.optionText}>
                    {prison?.address}, {prison?.states} {prison?.zipCode}
                  </Text>
                </View>
              </View>
            </View>
