...
const ArtistUpdateModal = ({ artist, onClose }) => {
  ...
  useEffect(() => {
    ...
    SetSelectedState({
      value: artist?.billingInfo?.states,
      label: statesOfUsa?.[artist?.billingInfo?.states],
    });
  ...
  const handleCreateLogin = (e, onClose) => {
    ...
    const promise = axios
      .patch(
        `https://mbb-e-commerce-server.vercel.app/createLogin/${artist?._id}`,
        credentials
      )
      .then((res) => {
        console.log(res.data);
          ...
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <form
                  className="space-y-4"
                  onSubmit={(e) => handleCreateLogin(e, onClose)}
                >
                  <div>
                    <label htmlFor="email">Artist Email</label>
                    <input
